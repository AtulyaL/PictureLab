...
private static void testGetCount() {
	 IntArrayWorker worker = new IntArrayWorker();
	    ...
	    worker.setMatrix(nums2);
	    int test = worker.getCount(1);
	  worker.print();
	    System.out.println("There should two 1s  in the array and there are: "+test);
}

private static void testGetColTotal() {
	 IntArrayWorker worker = new IntArrayWorker();
	    int [][] nums2 = {{1, 2, 3}, {4, 5, 6}};
	    worker.setMatrix(nums2);
	    int total = worker.getColTotal(1);
	   worker.print();
	    System.out.println("Column total for column one should be 7 and is: "+total);
}
...
private static void testGetLargest() {
	 IntArrayWorker worker = new IntArrayWorker();
	    ...
	    int largest = worker.getLargest();
	    worker.print();
	    System.out.println("largest num should be 6 and is: "+largest);
}
}